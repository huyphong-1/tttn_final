...
import { supabase } from "../../lib/supabaseClient"; // ✅ dùng singleton client
...
const AdminDashboard = () => {
  ...
  const fetchRecentOrders = useCallback(async (page = currentPageRef.current) => {
    try {
      ...
      const { data, error, count } = await supabase
        .from("orders")
        .select(
          "id, total_amount, status, created_at, customer_name, customer_email",
          { count: "exact" }
        )
  ...
  const fetchStats = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      const totalViews = productData.reduce(
        (sum, product) => sum + Number(product.view_count ?? 0),
        0
      );
  ...
  useEffect(() => {
    ...
    const channel = supabase
        ...
        if (isInCurrentMonth(payload.new?.created_at, new Date())) {
          ...
          if (currentPageRef.current === 1) {
            setRecentOrders((prev) =>
              [formatted, ...prev.filter((o) => o.id !== formatted.id)].slice(0, PAGE_SIZE)
            );
          }
        }
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "orders" }, (payload) => {
        ...
        setRecentOrders((prev) => {
          if (!isCurrentMonth) return prev.filter((o) => o.id !== payload.new?.id);

          return prev.map((o) =>
            o.id === payload.new?.id
              ? {
                  ...o,
                  total: payload.new?.total_amount || o.total,
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles" }, (payload) => {
        ...
        setStats((prev) => ({
          ...prev,
          loginsToday: Math.max(0, prev.loginsToday + (isToday ? 1 : -1)),
        }));
      })
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "dashboard_metrics", filter: `id=eq.${METRICS_ROW_ID}` },
        (payload) => {
  ...
  // ✅ Dark-friendly pills (no light backgrounds)
  ...
  // ✅ Force dark background even if some wrapper/page CSS is white
  if (!isAdmin) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Dashboard Quản Trị</h1>
          ...
          <div className="bg-slate-800 rounded-lg p-6 border border-slate-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.loginsToday.toLocaleString()}</p>
              </div>
        ...
        <div className="bg-slate-800 rounded-lg border border-slate-700">
                ...
                {recentOrders.length === 0 ? (
                  ...
                  recentOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-slate-700/50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-white">{formatPrice(order.total)}</td>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 px-6 py-4 border-t border-slate-700">
            <p className="text-sm text-slate-400">
              {hasOrders ? `Hiển thị ${rangeStart}-${rangeEnd} / ${recentOrdersCount} đơn hàng` : "Không có đơn hàng nào."}
            </p>
